...
const Health = () => {
  ...
  const moodOptions = [
    { emoji: "😊", label: t("health.moodGreat") },
    { emoji: "🙂", label: t("health.moodGood") },
    { emoji: "😐", label: t("health.moodOkay") },
    { emoji: "😔", label: t("health.moodLow") },
    { emoji: "😰", label: t("health.moodAnxious") },
  ];
  ...
  const handleLog = () => { setLogged(true); setTimeout(() => setLogged(false), 3000); };
  ...
  const getGlucoseTip = (value: string, type: "fasting" | "post") => {
    const num = parseFloat(value);
    if (!num) return null;
    if (type === "fasting" && num > 95) return t("health.tipFasting");
    if (type === "post" && num > 140) return t("health.tipPost");
    return null;
  };

  return (
    <div className="pt-8 pb-4 animate-fade-in">
      <h1 className="text-2xl font-display font-bold mb-1">{t("health.title")}</h1>
      ...
      <Tabs defaultValue="log" className="w-full">
        <TabsList className="grid w-full grid-cols-3 rounded-xl bg-muted/50 p-1 h-auto">
          <TabsTrigger value="log" className="rounded-lg text-xs py-2 data-[state=active]:bg-card data-[state=active]:shadow-soft">{t("health.tabLog")}</TabsTrigger>
          <TabsTrigger value="trends" className="rounded-lg text-xs py-2 data-[state=active]:bg-card data-[state=active]:shadow-soft">{t("health.tabTrends")}</TabsTrigger>
          <TabsTrigger value="insights" className="rounded-lg text-xs py-2 data-[state=active]:bg-card data-[state=active]:shadow-soft">{t("health.tabInsights")}</TabsTrigger>
        </TabsList>

        <TabsContent value="log" className="mt-4 space-y-5">
          <div className="bg-card rounded-2xl p-4 shadow-soft space-y-4">
            <h3 className="font-display font-semibold text-sm">{t("health.glucoseTitle")}</h3>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-xs text-muted-foreground">{t("health.fasting")}</Label>
              ...
              <div>
                <Label className="text-xs text-muted-foreground">{t("health.postMeal")}</Label>
          ...
          <div className="bg-card rounded-2xl p-4 shadow-soft">
            <h3 className="font-display font-semibold text-sm mb-3">{t("health.weight")}</h3>
            <Input type="number" value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="e.g. 68" className="rounded-xl" />
          </div>

          <div className="bg-card rounded-2xl p-4 shadow-soft">
            <h3 className="font-display font-semibold text-sm mb-3">{t("health.feelingTitle")}</h3>
          ...
          <Button onClick={handleLog} className="w-full rounded-xl" disabled={logged}>
            {logged ? t("health.logged") : t("health.saveLog")}
          </Button>
        </TabsContent>
...
const WeeklyTrends = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-card rounded-2xl p-4 shadow-soft">
        <h3 className="font-display font-semibold text-sm mb-4">{t("health.fastingGlucose")}</h3>
        ...
        <div className="flex items-center gap-4 mt-3 text-[10px]">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-range" /> {t("health.inRange")}</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-watch" /> {t("health.watch")}</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-coral" /> {t("health.adjust")}</span>
        </div>
      </div>

      <div className="bg-card rounded-2xl p-4 shadow-soft">
        <h3 className="font-display font-semibold text-sm mb-4">{t("health.postMealGlucose")}</h3>
        <div className="flex items-end justify-between gap-1 h-32">
          {days.map((day, i) => (
            <div key={day} className="flex flex-col items-center gap-1 flex-1">
      ...
      <p className="text-xs text-center text-muted-foreground">{t("health.weekSummary")}</p>
    </div>
  );
};
